...
class PWMetrics {
  ...
  findMedianRun(results) {
    const maxTimingValue = Math.max(...results.reduce((acc, r) => acc.concat(r.timings.map(t => t.timing)), []));
    ...
    const medianTTI = this.getMedianRun(metricValues);
    ...
    const medianRun = results.find(result => result.timings.find(timing =>
      timing.id === largestMetric.id && timing.timing === medianTTI
    ));
    return medianRun;
  }
